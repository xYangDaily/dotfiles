...
function vi-easy-motion () {
    ...
    function _easy-motion-main {
        ...
        "${_EASY_MOTION_ROOT_DIR}/easy_motion.py" "${EASY_MOTION_TARGET_KEYS}" "${CURSOR}" "${is_in_viopp}" "${move_mode}" "${BUFFER}" </dev/tty 2>/dev/null | \
            ...
            case "${state}" in
                ...
                jump)
                    ...
                    if [[ -n "${target_index}" ]]; then
                      if   [ "$move_mode" = "no" ] || [ "$move_mode" = "b" ] || [ "$move_mode" = "w" ]; then 
                        CURSOR="$((target_index))"
                      elif [ "$move_mode" = "ge" ] || [ "$move_mode" = "e" ]; then 
...
zle -N vi-easy-trigger
zle -N vi-easy-trigger-w
zle -N vi-easy-trigger-b
zle -N vi-easy-trigger-e
zle -N vi-easy-trigger-ge
zle -N vi-change
zle -N vi-delete
zle -N vi-down-case
