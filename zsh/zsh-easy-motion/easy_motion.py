...
class MissingModeFlagError(Exception):
    pass
...
def parse_arguments():
    ...
    # Extract move mode flag
    if not argv:
        raise MissingModeFlagError("No move mode given.")
    move_mode = argv.pop(0)
    if len(move_mode) > 2:
        raise MissingModeFlagError("invalid move mode")
    
    # Extract text
    if not argv:
        raise MissingTextError("No text given.")
    text = " ".join(argv)
    return cursor_position, is_in_viopp, target_keys, text, move_mode
...
def handle_user_input(cursor_position, is_in_viopp, target_keys, text, move_mode):
    ...
    try:
        while True:
            ...
            if read_state != ReadState.HIGHLIGHT:
                if read_state == ReadState.MOTION and move_mode != "no":
                    next_key = move_mode
                else:
...
def main():
    # type: () -> None
    try:
        cursor_position, is_in_viopp, target_keys, text, move_mode = parse_arguments()
        handle_user_input(cursor_position, is_in_viopp, target_keys, text, move_mode)
    except (
        MissingTargetKeysError,
        MissingCursorPositionError,
        InvalidCursorPositionError,
        MissingVioppFlagError,
        MissingModeFlagError,
